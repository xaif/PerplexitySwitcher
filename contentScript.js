(function() {
    ...
    function initSwitcher() {
      ...
      if (currentURL.includes('google.') && currentURL.includes('/search')) {
        ...
        const urlParams = new URLSearchParams(window.location.search);
        const query = urlParams.get('q') || '';
        chrome.storage.local.set({ previousSearchEngine: 'duckduckgo' }, function() {
          updateButton(query);
        });
      }
    }
    ...
    function injectButton(query, currentSite) {
      ...
      duckduckgoIcon.src = chrome.runtime.getURL('icons/duckduckgo-icon.svg');
      duckduckgoIcon.title = 'Search on DuckDuckGo';
      duckduckgoIcon.alt = 'DuckDuckGo Icon';
      ...
      if (currentSite === 'google') {
        ...
        if (previousSearchEngine === 'google') {
          ...
          icons = [braveIcon, perplexityIcon];
        } else if (previousSearchEngine === 'duckduckgo') {
          icons = [duckduckgoIcon, perplexityIcon];
        } else {
          icons = [googleIcon, perplexityIcon];
        }
      } else if (currentSite === 'duckduckgo') {
        icons = [duckduckgoIcon, perplexityIcon];
      }
      ...
      icons.forEach(icon => {
        if (
          ...
          (currentSite === 'duckduckgo' && icon === duckduckgoIcon)
        ) {
      ...
      if (icons.includes(duckduckgoIcon)) {
        duckduckgoIcon.addEventListener('mouseover', function() {
          duckduckgoIcon.style.transform = 'scale(1.1)';
        });
        duckduckgoIcon.addEventListener('mouseout', function() {
          duckduckgoIcon.style.transform = 'scale(1)';
        });
        duckduckgoIcon.addEventListener('click', function(event) {
          event.stopPropagation();
          if (currentSite !== 'duckduckgo') {
            chrome.storage.local.set({ previousSearchEngine: currentSite }, function() {
